...
<div class="main-div">
    <form name="main_form" method="POST" action="/index.php/Admin/Category/edit/cat_id/22/p/1.html" enctype="multipart/form-data" >
              ...
              <tr>
                ...
                <td>
                    ...
                    <?php if($searchAttrData): ?>
                        <?php foreach ($searchAttrData as $k1 => $v1): if($k1 == 0) $op = "[+]"; else $op = "[-]"; ?>
                        <li>
                        <a href="javascript:void(0);" onclick="addNed(this);"><?php echo ($op); ?></a>
                            <select name="type_id[]">
                                <option value="">-请选择类型-</option>
                                <?php foreach ($typeM as $k => $v): if($v['type_id'] == $v1['type_id']) $selected = 'selected="selected"'; else $selected = ''; ?>
                                <option <?php echo ($selected); ?> value="<?php echo ($v["type_id"]); ?>"><?php echo ($v["type_name"]); ?></option>
                                <?php endforeach;?>
                            </select>

                            <select attr_id="<?php echo ($v1["attr_id"]); ?>" name="attr_id[]">
                                <option>-请选择属性-</option>
                            </select>
                        </li>
                        <?php endforeach;?>                    
