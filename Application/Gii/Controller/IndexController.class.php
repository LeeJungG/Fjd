...
class IndexController extends Controller 
{
	public function index()
	{
		if(IS_POST)
		{
			...
			$topPriId = $priModel->field('pri_id')->where(array('pri_name' => array('eq',$topPriName)))->find();

			$has = $priModel->field('pri_id')->where(array('pri_name' => array('eq',$config['tableCnName'].'列表')))->count();
			if($has == 0)
			{
			//添加权限
			$lisPriId = $priModel->add(
				array(
					'pri_name' => $config['tableCnName'].'列表',
					'module_name' => $config['moduleName'],
					'controller_name' => $tpName,
					'action_name' => 'lst',
					'parent_id' => $topPriId['pri_id'],
					));
				$priModel->add(
				array(
